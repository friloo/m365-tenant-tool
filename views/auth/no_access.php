...
<div class="login-page">
    <div class="login-card text-center">
        ...
        <h5 class="mb-2 fw-bold"><?= te('Kein Zugriff') ?></h5>
        <p class="text-muted small mb-3">
            <?= te('Ihr Microsoft-Konto') ?>
            <?php if (!empty($upn)): ?>
                (<strong><?= htmlspecialchars($upn) ?></strong>)
            <?php endif; ?>
            <?= te('hat keinen Zugriff auf dieses Tool.') ?>
        </p>
        <p class="text-muted small mb-4">
            <?= te('Bitte wenden Sie sich an den Administrator, um Zugriff zu erhalten.') ?>
        </p>
