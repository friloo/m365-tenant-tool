...
<p class="text-muted mb-4" style="font-size:14px;">
    Microsoft 365 Gruppen ohne Aktivität in den letzten <?= (int)$days ?> Tagen
</p>

<!-- Days filter + Export -->
<div class="d-flex align-items-center gap-2 mb-4 flex-wrap">
    <label for="daysSelect" style="font-size:13px;font-weight:500;white-space:nowrap;">Inaktiv seit mehr als:</label>
...
<div class="alert alert-warning mb-4">
    ...
    <strong>Viele inaktive Gruppen</strong> — Viele inaktive Gruppen können auf ungenutzte Ressourcen und potenzielle
    Sicherheitsrisiken hinweisen. Erwägen Sie eine Bereinigung.
</div>
<?php endif; ?>
...
<div class="row g-3 mb-4">
    <div class="col-sm-3">
        <div class="metric-card">
            <div class="metric-label">Inaktive Gruppen gesamt</div>
            <div class="metric-value" style="color:<?= $totalCount > 0 ? '#d97706' : '#111827' ?>;">
                <?= $totalCount ?>
            </div>
            <div class="metric-sub">Seit &gt;<?= (int)$days ?> Tagen</div>
        </div>
    </div>
    <div class="col-sm-3">
        <div class="metric-card">
            <div class="metric-label">Nie aktiv</div>
            <div class="metric-value" style="color:<?= $neverActive > 0 ? '#dc2626' : '#16a34a' ?>;">
                <?= $neverActive ?>
            </div>
            <div class="metric-sub">Kein Aktivitätsdatum</div>
        </div>
    </div>
    <div class="col-sm-3">
        <div class="metric-card">
            <div class="metric-label">Durchschn. Inaktivitätsdauer</div>
            <div class="metric-value" style="color:#6b7280;">
                <?= $avgInactive ?>
            </div>
            <div class="metric-sub">Tage (ohne „Nie aktiv")</div>
        </div>
    </div>
    <div class="col-sm-3">
        <div class="metric-card">
            <div class="metric-label">Externe Mitglieder</div>
            <div class="metric-value" style="color:<?= $externalSum > 0 ? '#d97706' : '#111827' ?>;">
                <?= $externalSum ?>
            </div>
            <div class="metric-sub">In inaktiven Gruppen</div>
        </div>
    </div>
</div>
...
<div class="content-card">
    <div class="table-toolbar">
        <input type="text" id="groupSearch" class="search-box" placeholder="Gruppe suchen…">
    </div>
                ...
                <tr>
                    ...
                    <td>
                        <?php if ($row['last_activity'] === null): ?>
                            <span class="badge-danger">Nie aktiv</span>
                        <?php else: ?>
                    ...
                    <td>
                        ...
                        <?php if ($row['external_count'] > 0): ?>
                            <span class="badge-warning ms-1">+<?= (int)$row['external_count'] ?> extern</span>
                        <?php endif; ?>
                    </td>
                    <td>
                        <?php if ($row['group_id']): ?>
                ...
                <tr>
                    <td colspan="6">
                        <div class="empty-state">
                            ...
                            <p>Keine inaktiven Gruppen gefunden — alle Gruppen waren in den letzten <?= (int)$days ?> Tagen aktiv.</p>
                        </div>
                    </td>
                </tr>
                <?php endif; ?>
