...
$relativeSync = function (string $dt): string {
    ...
    $ts   = strtotime($dt);
    $diff = time() - $ts;
    if ($diff < 60)         return te('Gerade eben');
    if ($diff < 3600)       return te('Vor :n Minuten', ['n' => floor($diff / 60)]);
    if ($diff < 86400)      return te('Vor :n Stunden', ['n' => floor($diff / 3600)]);
    $days = floor($diff / 86400);
    return $days === 1.0 ? te('Vor 1 Tag') : te('Vor :n Tagen', ['n' => $days]);
};
...
<div class="mb-3">
    <a href="/devices" class="text-muted text-decoration-none small"><i class="bi bi-arrow-left me-1"></i><?= te('Zurück zu Geräte') ?></a>
</div>
        ...
        <div class="row g-3">
            ...
            <div class="col-lg-6">
                ...
                <table class="table table-sm mb-0">
                    <tbody>
                        <?php foreach ([
                            te('Hersteller')    => $detail['manufacturer'] ?? null,
                            te('Modell')        => $detail['model'] ?? null,
                            te('Seriennummer')  => $detail['serialNumber'] ?? null,
                            'IMEI'              => $detail['imei'] ?? null,
                            te('Benutzer')      => $detail['userDisplayName'] ?? null,
                            'UPN'               => $detail['userPrincipalName'] ?? null,
                            te('Registriert')   => $enrolledDate !== '–' ? $enrolledDate : null,
                            te('Verwaltung')    => $detail['managementState'] ?? null,
                        ] as $label => $val):
            ...
            <div class="col-lg-6">
                <div class="row g-2">
                    ...
                    <div class="col-sm-6">
                        <div class="metric-card text-center">
                            <div class="metric-label"><?= te('Verschlüsselung') ?></div>
                            <div class="metric-value" style="font-size:1.2rem;">
                                <?php if ($isEncrypted): ?>
                                    <span class="badge-enabled"><i class="bi bi-lock me-1"></i><?= te('Aktiv') ?></span>
                                <?php else: ?>
                                    <span class="badge-warning"><i class="bi bi-unlock me-1"></i><?= te('Inaktiv') ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-6">
                        <div class="metric-card text-center">
                            <div class="metric-label"><?= te('Freier Speicher') ?></div>
                            <div class="metric-value">
                                <?= $e($formatBytes($freeStorage)) ?>
                                <?php if ($totalStorage > 0): ?>
                                    <div class="text-muted" style="font-size:11px;"><?= te('von') ?> <?= $e($formatBytes($totalStorage)) ?></div>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-6">
                        <div class="metric-card text-center">
                            <div class="metric-label"><?= te('Letzter Sync') ?></div>
        ...
        <div class="row g-3">
            ...
            <div class="col-md-4">
                <div class="p-3 rounded h-100" style="background:#f0f7ff;border:1px solid #cce0ff;">
                    <h6 class="fw-semibold mb-1"><i class="bi bi-arrow-repeat text-primary me-1"></i><?= te('Synchronisieren') ?></h6>
                    <p class="text-muted small mb-3"><?= te('Fordert das Gerät auf, sich sofort mit Intune zu synchronisieren.') ?></p>
                    <form method="post" action="/devices/<?= $e($deviceId) ?>/sync">
                        <?= \App\Core\Csrf::field() ?>
                        <button type="submit" class="btn btn-sm btn-primary">
                            <i class="bi bi-arrow-repeat me-1"></i><?= te('Sync anfordern') ?>
                        </button>
                    </form>
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="p-3 rounded h-100" style="background:#fff8f0;border:1px solid #ffd8b0;">
                    ...
                    <p class="text-muted small mb-3"><?= te('Entfernt Unternehmensdaten, lässt persönliche Daten intakt. Geeignet für persönliche Geräte (BYOD).') ?></p>
                    <form method="post" action="/devices/<?= $e($deviceId) ?>/retire"
                          onsubmit="return confirm('<?= t('Gerät wirklich zurücksetzen (Retire)? Unternehmensdaten werden entfernt, persönliche Daten bleiben erhalten.') ?>')">
                        <?= \App\Core\Csrf::field() ?>
                        <button type="submit" class="btn btn-sm btn-warning">
                            <i class="bi bi-box-arrow-right me-1"></i><?= te('Retire ausführen') ?>
                        </button>
                    </form>
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="p-3 rounded h-100" style="background:#fff5f5;border:1px solid #ffd5cc;">
                    <h6 class="fw-semibold mb-1"><i class="bi bi-trash text-danger me-1"></i><?= te('Wipe (Werksreset)') ?></h6>
                    <p class="text-muted small mb-3"><?= te('Löscht alle Daten auf dem Gerät. Nicht rückgängig machbar. Nur für Unternehmensgeräte.') ?></p>
                    <form method="post" action="/devices/<?= $e($deviceId) ?>/wipe"
                        ...
                        <button type="submit" class="btn btn-sm btn-danger">
                            <i class="bi bi-trash me-1"></i><?= te('Wipe ausführen') ?>
                        </button>
                    </form>
                </div>
            </div>
    ...
    <div class="card-body-custom">
        ...
        <?php else: ?>
            <div class="alert alert-warning small mb-3 py-2">
                ...
                <strong><?= te('Hinweis:') ?></strong> <?= te('Schlüssel werden nur protokolliert angezeigt. Stelle sicher, dass der Zugriff nur autorisierten Personen möglich ist.') ?>
            </div>
                ...
                <table class="data-table">
                    <thead>
                        <tr>
                            ...
                            <th><?= te('Schlüssel-ID') ?></th>
                            <th><?= te('Recovery-Schlüssel') ?></th>
                        </tr>
                    </thead>
                    <tbody>
                                ...
                                <td>
                                    <?php if (!empty($bk['key'])): ?>
                                        ...
                                        <button
                                            ...
                                            title="<?= te('Schlüssel kopieren') ?>"
                                        ><i class="bi bi-clipboard"></i></button>
                                    <?php else: ?>
                                        <span class="text-muted small"><?= te('Schlüsselwert nicht verfügbar') ?></span>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
