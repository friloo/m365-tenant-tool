...
function _wfActionRow(int $idx, string $type, array $cfg, array $actions): string {
    $opts = '<option value="">— wählen —</option>';
...
<div class="content-card">
    <h1 class="mb-2"><i class="bi bi-diagram-2"></i> <?= $existing ? 'Workflow bearbeiten' : 'Neuer Workflow' ?></h1>
    <p class="text-muted small"><a href="/workflows">← zurück zur Übersicht</a></p>
    ...
    <form method="post" action="/workflows/save" id="wfForm">
        ...
        <div class="card mb-3">
            <div class="card-header bg-light"><strong>Trigger-Konfiguration</strong></div>
            <div class="card-body">
                <div class="trig-cfg" id="trig-schedule" style="<?= ($existing['trigger_key'] ?? 'schedule') === 'schedule' ? '' : 'display:none;' ?>">
                    <label class="form-label small">Intervall (Minuten, mindestens 15)</label>
                ...
                <div class="trig-cfg" id="trig-new_guest_user" style="<?= ($existing['trigger_key'] ?? '') === 'new_guest_user' ? '' : 'display:none;' ?>">
                    <p class="text-muted small mb-0">Keine Konfiguration nötig — der Trigger feuert für jeden neuen Gast-Benutzer seit dem letzten Lauf.</p>
                </div>
                <div class="trig-cfg" id="trig-new_user_in_group" style="<?= ($existing['trigger_key'] ?? '') === 'new_user_in_group' ? '' : 'display:none;' ?>">
                    <label class="form-label small">Gruppen-ID (GUID aus /groups)</label>
        ...
        <div class="card mb-3">
            <div class="card-header bg-light d-flex justify-content-between align-items-center">
                ...
                <button type="button" class="btn btn-sm btn-outline-primary" onclick="addAction()"><i class="bi bi-plus-lg"></i> Aktion hinzufügen</button>
            </div>
            <div class="card-body" id="actionsList">
                ...
                <?php if (empty($existingActions)): ?>
                    <p class="text-muted small mb-0" id="noActionsHint">Noch keine Aktionen — bitte mindestens eine hinzufügen.</p>
                <?php endif; ?>
            </div>
        </div>
    ...
    <?php if ($existing): ?>
        <div class="d-flex gap-2 mt-3">
            <form method="post" action="/workflows/<?= (int)$existing['id'] ?>/run-now">
                <?= Csrf::field() ?>
                <button type="submit" class="btn btn-outline-success"><i class="bi bi-play"></i> Jetzt ausführen</button>
            </form>
            <form method="post" action="/workflows/<?= (int)$existing['id'] ?>/delete"
                  onsubmit="return confirm('Workflow wirklich löschen?');" class="ms-auto">
                <?= Csrf::field() ?>
                <button type="submit" class="btn btn-outline-danger"><i class="bi bi-trash"></i> Löschen</button>
            </form>
        </div>
    <?php endif; ?>
</div>
...
<?php if ($existing && !empty($runs)): ?>
    <div class="content-card mt-3">
        ...
        <table class="table table-sm">
            <thead><tr><th>Zeit</th><th>Ziel</th><th>Status</th><th>Detail</th></tr></thead>
            <tbody>
...
function renderActionFields(select, idx) {
    ...
    html += '<p class="small text-muted mb-0">Verfügbare Variablen: <code>{{user.userPrincipalName}}</code>, <code>{{user.displayName}}</code>, <code>{{user.id}}</code>, <code>{{trigger}}</code></p>';
