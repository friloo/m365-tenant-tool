...
<div class="alert alert-info d-flex gap-3 mb-3">
    ...
    <div>
        ...
        <strong><?= te('Voraussetzung:') ?></strong> <?= te('Microsoft 365 E5 oder als Add-on, plus Bedingung für viele
        DSGVO-Verträge mit Mandanten in regulierten Branchen.') ?>
    </div>
</div>

<div class="content-card mb-4">
    <div class="card-header-custom"><i class="bi bi-gear text-primary"></i><h6><?= te('Status (manuell gepflegt)') ?></h6></div>
                ...
                <div class="col-md-12">
                    <div class="form-check form-switch">
                        ...
                        <label class="form-check-label fw-medium" for="lockboxEnabled">
                            <?= te('Customer Lockbox ist im Tenant aktiviert') ?>
                        </label>
                    </div>
                </div>
                <div class="col-md-8">
                    <label class="form-label fw-medium"><?= te('Approver-Liste') ?> <span class="text-muted small"><?= te('(UPNs, kommagetrennt)') ?></span></label>
                    <input type="text" name="approvers" class="form-control" value="<?= $e($data['approvers']) ?>"
                           placeholder="admin1@firma.de, admin2@firma.de">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-medium"><?= te('Reaktions-SLA (Stunden)') ?></label>
                    <input type="number" name="sla_hours" class="form-control" min="0" max="72"
                           value="<?= (int)$data['sla_hours'] ?>">
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-medium"><?= te('Letzte Review') ?></label>
                    <input type="date" name="last_review" class="form-control" value="<?= $e($data['last_review']) ?>">
                    <div class="form-text"><?= te('Halbjährlich prüfen, ob noch alles aktuell ist.') ?></div>
                </div>
            </div>
...
<div class="content-card mb-4">
    <div class="card-header-custom"><i class="bi bi-link-45deg text-info"></i><h6><?= te('Konfiguration im Admin-Center') ?></h6></div>
    <div class="card-body-custom">
        <p class="text-muted small mb-3">
            <?= te('Customer Lockbox wird im Microsoft 365 Admin Center konfiguriert. Microsoft Graph stellt
        ...
        <div class="d-flex flex-wrap gap-2">
            ...
            <a href="https://learn.microsoft.com/de-de/purview/customer-lockbox-requests" target="_blank" rel="noopener" class="btn btn-outline-secondary">
                <i class="bi bi-book me-1"></i><?= te('Microsoft-Doku') ?>
            </a>
        </div>
    </div>
</div>
