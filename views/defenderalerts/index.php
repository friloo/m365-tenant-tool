...
<div class="row g-3 mb-4">
    <div class="col-sm-3">
        <div class="metric-card">
            <div class="metric-label"><?= te('Gesamt offen') ?></div>
            <div class="metric-value"><?= count($alerts) ?></div>
            <div class="metric-sub"><?= te('Aktive Warnungen') ?></div>
        </div>
    </div>
    <div class="col-sm-3">
        <div class="metric-card">
            <div class="metric-label"><?= te('Kritisch') ?></div>
    ...
    <div class="col-sm-3">
        <div class="metric-card">
            <div class="metric-label"><?= te('Mittel') ?></div>
            <div class="metric-value" style="color:<?= ($stats['bySeverity']['medium'] ?? 0) > 0 ? '#d97706' : '#111827' ?>;">
                <?= $stats['bySeverity']['medium'] ?? 0 ?>
            </div>
            <div class="metric-sub"><?= te('Mittlere Schwere') ?></div>
        </div>
    </div>
    <div class="col-sm-3">
        <div class="metric-card">
            <div class="metric-label"><?= te('Niedrig') ?></div>
            <div class="metric-value"><?= $stats['bySeverity']['low'] ?? 0 ?></div>
            <div class="metric-sub"><?= te('Niedrige Schwere') ?></div>
        </div>
    </div>
</div>
    ...
    <div class="content-card">
        <div class="card-body-custom">
            ...
            if (!empty($diag ?? null)) {
                ...
                $diagTitle = t('Keine Defender-Daten verfügbar');
                include BASE_PATH . '/views/partials/graph_diagnostic.php';
            } else { ?>
                <div class="empty-state">
                    <i class="bi bi-shield-slash"></i>
                    <p><?= te('Aktuell keine Defender-Warnungen — alles ruhig.') ?></p>
                </div>
            <?php } ?>
        </div>
    </div>
<?php else: ?>
    ...
    <?php if ($highCount > 0): ?>
        <div class="alert alert-danger d-flex align-items-center mb-4">
            ...
            <strong><?= te(':n kritische Sicherheitswarnungen erfordern sofortige Aufmerksamkeit', ['n' => $highCount]) ?></strong>
        </div>
    <?php endif; ?>
    ...
    <div class="content-card">
        <div class="card-header-custom">
            <span><i class="bi bi-shield-exclamation me-2"></i><?= te('Aktive Sicherheitswarnungen') ?></span>
        </div>
        <div class="card-body-custom">
            <div class="table-toolbar">
                <input type="text" id="alertsSearch" class="search-box" placeholder="<?= te('Warnungen suchen…') ?>">
            </div>
                ...
                <table class="data-table" id="alertsTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($status === 'new'): ?>
                                    <span class="badge-danger badge-pill"><?= te('Neu') ?></span>
                                <?php elseif ($status === 'inProgress'): ?>
                                    <span class="badge-warning badge-pill"><?= te('In Bearbeitung') ?></span>
                                <?php elseif ($status === 'resolved'): ?>
                                    <span class="badge-success badge-pill"><?= te('Gelöst') ?></span>
                                <?php else: ?>
                            ...
                            <td>
                                <?php if ($status !== 'resolved'): ?>
                                    <form method="POST"
                                          ...
                                          onsubmit="return confirm('<?= te('Warnung als gelöst markieren?') ?>');">
                                        <?= \App\Core\Csrf::field() ?>
                                        <button type="submit"
                                                class="btn btn-sm btn-outline-success"
                                                style="font-size:11px;padding:2px 8px;">
                                            <i class="bi bi-check2 me-1"></i><?= te('Lösen') ?>
                                        </button>
                                    </form>
                                <?php else: ?>
