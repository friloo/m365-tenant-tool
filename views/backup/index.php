...
<div class="alert alert-warning d-flex gap-3 mb-3">
    ...
    <div>
        <strong><?= te('Microsoft 365 sichert deine Daten NICHT.') ?></strong>
        <?= te('Die Recycle-Bin-Frist von 30-93 Tagen ist kein Backup —
...
<div class="content-card mb-4" style="border-left: 4px solid <?= $health['score'] >= 75 ? '#16a34a' : ($health['score'] >= 50 ? '#d97706' : '#dc2626') ?>;">
    <div class="card-body-custom d-flex align-items-center gap-3 flex-wrap">
        ...
        <div class="flex-grow-1">
            <div class="fw-semibold" style="font-size:16px;"><?= te('Backup-Health-Score') ?></div>
            <div class="text-muted small">
                <?= $health['score'] >= 75 ? te('Backup-Setup ist gut konfiguriert.') :
                   ($health['score'] >= 50 ? te('Backup-Setup hat Lücken.') : te('Backup-Setup hat kritische Lücken oder fehlt komplett.')) ?>
            </div>
        </div>
        <?php if (!empty($data['provider'])): ?>
            <div class="text-end">
                <div class="text-muted small"><?= te('Anbieter') ?></div>
                <div class="fw-medium">
...
<div class="content-card mb-4">
    <div class="card-header-custom"><i class="bi bi-list-check text-warning"></i><h6><?= te('Findings') ?></h6></div>
...
<!-- ── Konfig-Form ───────────────────────────────────────────────────── -->
<div class="content-card mb-4">
    <div class="card-header-custom"><i class="bi bi-gear text-primary"></i><h6><?= te('Backup-Konfiguration eintragen') ?></h6></div>
    <div class="card-body-custom">
        <form method="post" action="/backup/save">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    <label class="form-label fw-medium"><?= te('Anbieter-URL') ?> <span class="text-muted small"><?= te('(optional)') ?></span></label>
                    <input type="url" name="provider_url" class="form-control" value="<?= $e($data['provider_url']) ?>"
                           placeholder="https://console.veeam.com/...">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-medium"><?= te('Letzter Backup-Lauf') ?></label>
                    <input type="datetime-local" name="last_run" class="form-control" value="<?= $e($data['last_run']) ?>">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-medium"><?= te('Status') ?></label>
                    <select name="last_run_status" class="form-select">
                        <option value=""><?= te('— wählen —') ?></option>
                        <?php foreach (['success' => te('Erfolgreich'), 'partial' => te('Teilweise erfolgreich'), 'failed' => te('Fehlgeschlagen')] as $k => $v): ?>
                            <option value="<?= $k ?>" <?= $data['last_run_status'] === $k ? 'selected' : '' ?>><?= $v ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-medium"><?= te('Aufbewahrung (Tage)') ?></label>
                    <input type="number" name="retention_days" class="form-control" min="0" max="3650"
                           value="<?= $e($data['retention_days']) ?>" placeholder="z.B. 365">
                </div>
                <div class="col-12">
                    <label class="form-label fw-medium"><?= te('Geschützte Workloads') ?></label>
                ...
                <div class="col-md-6">
                    <label class="form-label fw-medium"><?= te('Letzter Restore-Test') ?></label>
                    <input type="date" name="restore_tested" class="form-control" value="<?= $e($data['restore_tested']) ?>">
                    <div class="form-text"><?= te('Mindestens einmal jährlich.') ?></div>
                </div>
                <div class="col-12">
                    <label class="form-label fw-medium"><?= te('Notizen') ?> <span class="text-muted small"><?= te('(optional)') ?></span></label>
