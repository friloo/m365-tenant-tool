...
<div class="row g-3 mb-4">
    <?php
    $statCards = [
        ['label' => 'Gesamt', 'key' => 'total', 'icon' => 'link-45deg', 'color' => 'primary'],
        ['label' => 'Aktiv', 'key' => 'active', 'icon' => 'circle-fill', 'color' => 'success'],
        ['label' => 'Prüfung ausstehend', 'key' => 'pending_review', 'icon' => 'clock-history', 'color' => 'warning'],
        ['label' => 'Überfällig', 'key' => 'overdue', 'icon' => 'exclamation-triangle-fill', 'color' => 'danger'],
        ['label' => 'Bestätigt', 'key' => 'confirmed', 'icon' => 'check-circle-fill', 'color' => 'info'],
        ['label' => 'Widerrufen', 'key' => 'revoked', 'icon' => 'x-circle-fill', 'color' => 'secondary'],
    ];
...
<div class="content-card mb-4">
    <div class="card-body-custom d-flex gap-2 flex-wrap align-items-center">
        <a href="/sharing/monitor/scan" class="btn btn-sm btn-primary" onclick="return confirm('Alle SharePoint-Freigaben jetzt scannen? Dies kann einige Minuten dauern.')">
            <i class="bi bi-search me-1"></i>Jetzt scannen
        </a>
        <span class="text-muted" style="font-size:12px; margin-left:8px;">
            <i class="bi bi-info-circle me-1"></i>Alternativ Cron: <code>php run-share-monitor.php</code>
        </span>
        <div class="ms-auto">
...
<div class="content-card">
    <div class="card-header-custom d-flex align-items-center justify-content-between flex-wrap gap-2">
        <div class="d-flex align-items-center gap-2">
            ...
            <h6>Überwachte Freigaben</h6>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            <?php
            $filters = ['' => 'Alle', 'active' => 'Aktiv', 'pending_review' => 'Ausstehend', 'confirmed' => 'Bestätigt', 'revoked' => 'Widerrufen'];
            ...
            <table class="table table-hover align-middle mb-0" id="shareTable">
                ...
                <?php if (empty($shares)): ?>
                    <tr><td colspan="8" class="text-center text-muted py-4">
                        <i class="bi bi-inbox me-2"></i>Keine Freigaben gefunden.
                        <?php if (!$statusFilter): ?>
                ...
                <?php foreach ($shares as $s):
                    ...
                    $statusLabel = match($s['status']) {
                        'active'         => 'Aktiv',
                        'pending_review' => 'Prüfung läuft',
                        'confirmed'      => 'Bestätigt',
                        'revoked'        => 'Widerrufen',
                        default          => $e($s['status']),
                    };
                    $scopeLabel = match($s['share_scope']) {
                        'anonymous'    => '<span class="badge bg-danger">🌐 Öffentlich</span>',
                        'users'        => '<span class="badge bg-warning text-dark">👥 Extern</span>',
                        'organization' => '<span class="badge bg-info text-dark">🏢 Org</span>',
                ...
                <tr class="<?= $isOverdue ? 'table-danger' : '' ?>">
                    <td>
                        ...
                        <?php if ($s['last_review_reason']): ?>
                            <div class="text-muted" style="font-size:11px;" title="Begründung: <?= $e($s['last_review_reason']) ?>">
                        ...
                        <?php if ($s['next_review_at']): ?>
                            ...
                            <?php if ($s['auto_revoke_at']): ?>
                                <div class="text-danger" style="font-size:11px;">
                                    <i class="bi bi-clock-history me-1"></i>Widerruf: <?= date('d.m.Y', strtotime($s['auto_revoke_at'])) ?>
                                </div>
                            <?php endif; ?>
                        <?php else: ?>
                    ...
                    <td>
                        ...
                        <?php if ($isOverdue): ?>
                            <span class="badge bg-danger ms-1" title="Widerruf überfällig">!</span>
                        <?php endif; ?>
                    </td>
                    <td>
                        ...
                        <?php else: ?>
                            <span class="text-muted" style="font-size:12px;">Widerrufen</span>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php endforeach; ?>
