...
<div class="alert alert-info d-flex gap-3 mb-3">
    ...
    <div>
        <strong>Monatlicher Executive-Report</strong> per E-Mail an die Geschäftsführung mit den wichtigsten
        Tenant-KPIs: Security-Score, MFA-Quote, Risiko-Benutzer, Defender-Alerts, CA-Policies, Top-Findings
        aus der Posture-Analyse. Cron startet am 1. jedes Monats um 07:00 Uhr (lokale Zeitzone).
    </div>
</div>

<!-- ── Konfiguration ─────────────────────────────────────────────────── -->
<div class="content-card mb-4">
    <div class="card-header-custom"><i class="bi bi-gear text-primary"></i><h6>Aktivierung &amp; Empfänger</h6></div>
        ...
        <form method="post" action="/executivereport/save">
            ...
            <div class="form-check form-switch mb-3">
                <input class="form-check-input" type="checkbox" name="enabled" id="execEnabled" value="1" role="switch"
                       <?= $enabled ? 'checked' : '' ?>>
                <label class="form-check-label fw-medium" for="execEnabled">Monatlichen Executive-Report versenden</label>
            </div>
            <label class="form-label fw-medium">Empfänger <span class="text-muted small">(kommagetrennt)</span></label>
            <input type="text" name="recipients" class="form-control" value="<?= $e($recipients) ?>"
                   placeholder="cio@firma.de, gf@firma.de">
            <div class="form-text">Standard ist die Alert-Mail-Adresse aus den allgemeinen Einstellungen.</div>
...
<div class="content-card mb-4">
    <div class="card-header-custom"><i class="bi bi-eye text-info"></i><h6>Vorschau &amp; Test</h6></div>
    <div class="card-body-custom">
        <p class="text-muted small mb-3">
            So wird der Report aussehen wenn er versendet wird (mit den aktuellen Tenant-Daten).
            Der Test-Versand schickt die Mail jetzt ohne auf den 1. des Monats zu warten.
        </p>
        <div class="d-flex gap-2 flex-wrap">
            <a href="/executivereport/preview" target="_blank" rel="noopener" class="btn btn-outline-primary">
                <i class="bi bi-eye me-1"></i>Vorschau im Browser öffnen
            </a>
