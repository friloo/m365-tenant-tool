...
<div class="alert alert-warning d-flex gap-3 mb-3">
    ...
    <div>
        <strong>Externe Weiterleitung ist der häufigste Exfiltrations­vektor</strong> bei kompromittierten Konten.
        M365 kennt dafür <strong>zwei Mechanismen</strong>: die <em>Postfach-Weiterleitung</em> (am Postfach gesetzt)
        und <em>Posteingangsregeln</em>, die Mails weiterleiten/umleiten. Beide werden hier geprüft.
    </div>
</div>
...
<ul class="nav nav-tabs mb-3" role="tablist">
    <li class="nav-item" role="presentation">
        <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#tab-mbx" type="button" role="tab">
            <i class="bi bi-forward-fill me-1"></i>Postfach-Weiterleitung
    ...
    <li class="nav-item" role="presentation">
        <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab-rules" type="button" role="tab">
            <i class="bi bi-arrow-right-square me-1"></i>Posteingangsregeln
    ...
    <div class="tab-pane fade show active" id="tab-mbx" role="tabpanel">
        ...
        <?php if (empty($forwards)): ?>
            <div class="content-card"><div class="card-body-custom"><div class="empty-state">
                ...
                <p class="mt-3 mb-1 fw-medium">Keine Postfach-Weiterleitungen gefunden</p>
                <p class="text-muted small">Kein Postfach leitet per Postfach-Einstellung extern weiter — der gewünschte Zustand.</p>
            </div></div></div>
        <?php else: ?>
            <div class="content-card">
                ...
                <div class="table-responsive">
                    <table class="data-table" id="fwdTable">
                        <thead><tr>
                            <th>Benutzer</th><th>Weiterleitungsadresse</th><th>Status</th>
                            <th>Lokal&nbsp;+&nbsp;Weiterleiten</th><th class="text-end">Aktion</th>
                        </tr></thead>
                        <tbody>
                        <?php foreach ($forwards as $fwd): ?>
                            <tr>
                                ...
                                <td><?php if ($fwd['forwardingEnabled']): ?><span class="badge-danger badge-pill">Aktiv</span><?php else: ?><span class="badge-secondary badge-pill">Inaktiv</span><?php endif; ?></td>
                                <td><?php if ($fwd['deliverToMailboxAndForward']): ?><span class="badge-info badge-pill">Ja</span><?php else: ?><span class="badge-neutral badge-pill">Nein</span><?php endif; ?></td>
                                <td class="text-end">
                                    <form method="post" action="/mailboxes/external-forwards/remove"
                                        ...
                                        <button type="submit" class="btn btn-sm btn-outline-danger"><i class="bi bi-x-circle me-1"></i>Entfernen</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
        ...
        <div class="row g-3 mb-4">
            ...
            <div class="col-sm-6 col-lg-3"><div class="metric-card">
                ...
                <div class="metric-sub">
                    <?php if (($report['skipped_errors'] ?? 0) > 0): ?><?= $report['skipped_errors'] ?> nicht lesbar
                    <?php elseif (!empty($report['truncated'])): ?>Limit erreicht — erste 500
                    <?php else: ?>alle aktiven User<?php endif; ?>
                </div>
            </div></div>
        </div>

        <div class="content-card mb-4">
            <div class="card-header-custom"><i class="bi bi-arrow-right-square-fill text-danger"></i><h6>Externe Auto-Weiterleitungen</h6></div>
            <div class="card-body-custom p-0">
                <?php if (empty($report['external_forward'])): ?>
                    <div class="text-muted small p-4 text-center"><i class="bi bi-check-circle text-success me-1"></i>Keine Inbox-Regeln, die an externe Adressen weiterleiten.</div>
                <?php else: ?>
                    <div class="table-responsive"><table class="data-table">
                        <thead><tr><th>Benutzer</th><th>Regel-Name</th><th>Weiterleitung an</th><th>Status</th></tr></thead>
                        <tbody>
                        <?php foreach ($report['external_forward'] as $r): ?>
                            <tr>
                                ...
                                <td><?php if ($r['enabled']): ?><span class="badge bg-warning text-dark"><i class="bi bi-play-fill me-1"></i>Aktiv</span><?php else: ?><span class="badge bg-secondary">Inaktiv</span><?php endif; ?></td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
                    </table></div>
