...
<div class="row g-3 mb-4">
    <div class="col-sm-3">
        <div class="metric-card">
            <div class="metric-label">Gesamt Abonnements</div>
            <div class="metric-value"><?= $totalCount ?></div>
            <div class="metric-sub">Alle Lizenzen</div>
        </div>
    </div>
    <div class="col-sm-3">
        <div class="metric-card">
            <div class="metric-label">Ablaufend (≤60 Tage)</div>
            <div class="metric-value" style="color:<?= $expiringCount > 0 ? '#dc2626' : '#16a34a' ?>;">
                <?= $expiringCount ?>
            </div>
            <div class="metric-sub">
                <?php if ($expiringCount > 0): ?>
                    <span class="badge-danger"><?= $expiringCount ?> bald fällig</span>
                <?php else: ?>
                    Keine bald fällig
                <?php endif; ?>
            </div>
    ...
    <div class="col-sm-3">
        <div class="metric-card">
            <div class="metric-label">Kritisch (≤30 Tage)</div>
            <div class="metric-value" style="color:<?= $criticalCount > 0 ? '#dc2626' : '#16a34a' ?>;">
                <?= $criticalCount ?>
            </div>
            <div class="metric-sub">Sofortige Aufmerksamkeit</div>
        </div>
    </div>
    <div class="col-sm-3">
        <div class="metric-card">
            <div class="metric-label">Gesperrt/Warnung</div>
            <div class="metric-value" style="color:<?= $warningCount > 0 ? '#d97706' : '#16a34a' ?>;">
                <?= $warningCount ?>
            </div>
            <div class="metric-sub">Status nicht „Enabled"</div>
        </div>
    </div>
</div>
...
<div class="content-card">
    <div class="card-header-custom d-flex align-items-center justify-content-between">
        <span><i class="bi bi-calendar-check me-2"></i>Lizenz-Abonnements</span>
        <div class="d-flex gap-2">
            ...
            <a href="/licenses" class="btn btn-sm btn-outline-primary">
                <i class="bi bi-grid me-1"></i>Lizenzübersicht
            </a>
        </div>
    </div>
            ...
            <table class="data-table" id="expiryTable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($nextDt === null): ?>
                                <span class="text-muted" style="font-size:12px;">Nicht verfügbar</span>
                        ...
                        <td>
                            ...
                            <?php elseif ($daysLeft <= 30): ?>
                                <span class="badge-danger"><?= $daysLeft ?> Tage</span>
                            <?php elseif ($daysLeft <= 60): ?>
                                <span class="badge-warning"><?= $daysLeft ?> Tage</span>
                            <?php else: ?>
                                <span class="text-muted" style="font-size:12px;"><?= $daysLeft ?> Tage</span>
                            <?php endif; ?>
                        </td>
                    ...
                    <tr>
                        <td colspan="7">
                            <div class="empty-state">
                                ...
                                <p>Keine Abonnement-Informationen verfügbar.</p>
                            </div>
                        </td>
                    </tr>
                    <?php endif; ?>
